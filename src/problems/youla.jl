...
# prob0, b0, t0 = youlaOptimProblem()
# @time sol = Convex.solve!(prob0, SCSSolver(eps=1e-6))
# @time sol = Convex.solve!(prob0, GAPA(direct=true, eps=1e-6))
# @time sol = Convex.solve!(prob0, AndersonWrapper(DR(direct=true, eps=1e-6)))
# @time sol = Convex.solve!(prob0, AndersonWrapper(GAPA(1.0, 0.5, 0.9, direct=true, eps=1e-6)))

# smax = 1.7
# prob1, b1 = youlaProblem(problem=1, smax=smax)
# prob2, b2 = youlaProblem(problem=2, smax=smax)
# prob3 = youlaProblem(problem=3, smax=smax)

# solver = DR(checki=100, eps=1e-9, direct=false, max_iters=100000)
# @time sol = FirstOrderSolvers.solve!(prob3, solver)

# maximum(abs,prox(prob3.S1, sol[1].x)[1]-sol[1].x)
# maximum(abs,prox(prob3.S2, sol[1].x)[1]-sol[1].x)
# using SCS


# @time sol = Convex.solve!(prob1, SCSSolver(eps=1e-12))
# @time sol = Convex.solve!(prob2, SCSSolver())

# solver = DR(checki=10, eps=1e-9, direct=false, max_iters=100000)

# @time sol = Convex.solve!(prob1, solver)
# @time sol = Convex.solve!(prob2, solver)
# @time sol = FirstOrderSolvers.solve!(prob3, solver)

# solver = GAPA(checki=10, eps=1e-9, direct=true)

# @time sol = Convex.solve!(prob1, solver)
# @time sol = Convex.solve!(prob2, solver)
# @time sol = FirstOrderSolvers.solve!(prob3, solver)

# solver = FirstOrderSolvers.LongstepWrapper(GAPA(checki=10, eps=1e-9, direct=false))
...
function youlaOptimProblem(;problem=1, N = 40, N_t = 300, N_f = 150)
    ...
    prob = minimize(t3_max, c1, c3_re, c3_im, c4, c5, c7, c8, c9, c10, c11)
    return prob, b, t3_max
end


function youlaProblem(;problem=1, N = 40, N_t = 300, N_f = 150, smax=1.6)
    #P = c2d(1/(20*s + 1) * exp(-5*s), 1);
    z_d = tf("z", 1)
    P_sysd = (1/20)/(z_d-0.95)/z_d^5
    P_d(z) = (1/20)/(z-0.95)/z^5
    ...
    #prob = minimize(0, c1, c3_re, c3_im, c4, c5, c6, c7, c8, c9, c10, c11)
    prob = minimize(0, c1, c3_re, c3_im, c4, c5, c7, c8, c9, c10, c11)
    ...
    ball = [real(Sb_fr); imag(Sb_fr); -5^2; fill(smax^2, N_f); PSb; 1.1207+0.01]
    ...
    ball2 = [real(Sb_fr); imag(Sb_fr); -5; fill(smax, N_f); PSb; sqrt(1.1207+0.01)]
    ...
    prob = FirstOrderSolvers.Feasibility(S1, S2, size(Aall,2))
    if problem == 3
        return prob
    end
end
